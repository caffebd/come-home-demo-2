...
var can_start_throw_timer: bool = true
...
func _ready():
	...
	GlobalSignals.picked_up_timed_throw_item.connect(_picked_up_timed_throw_item)
...
func set_held_object(body):
	if body is RigidBody3D:
		if body.is_in_group("pick_item"):
			...
			if body.is_in_group("timed_throw"):
				_picked_up_timed_throw_item()
...
func throw_held_object():
	...
	if timed_throw:
		throw_force = 0.6
	obj.apply_central_impulse(-camera.global_basis.z * throw_force * 10)
...
func handle_holding_objects():
	# Throwing Objects
	if Input.is_action_just_pressed("throw") and not timed_throw:
		if heldObject != null: throw_held_object()
	
	if Input.is_action_pressed("throw") and timed_throw:
		if heldObject != null:
			GlobalSignals.emit_signal("hide_player_info")
			if can_start_throw_timer:
				hud.throw_timer.value = 0.0					
				can_start_throw_timer = false
				hud.throw_timer.visible = true
				%ThrowTimer.start()
	
	if Input.is_action_just_released("throw") and timed_throw:
		%ThrowTimer.stop()
		hud.throw_timer.value = 0.0					
		can_start_throw_timer = true
		#hud.throw_timer.visible = true	
...
func _picked_up_timed_throw_item():
	hud.throw_timer.value = 0.0
	hud.throw_timer.visible = true
	

func _on_throw_timer_timeout() -> void:
	hud.throw_timer.value += 0.1
	print (hud.throw_timer.value)
	if hud.throw_timer.value >= 5.9:
		throw_held_object()
		%ThrowTimer.stop()
		hud.throw_timer.visible = false
		hud.show_right_click.visible = false
		can_start_throw_timer = true
