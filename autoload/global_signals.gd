extends Node

signal start_game()

signal hud_menu(state)

signal to_night()

signal give_torch()
signal give_lamp()

signal dad_to_mound()
signal dad_to_clearing()

signal player_to_mound()

signal start_clearing()

signal stick_drop()

signal start_house()
signal start_in_cave()

signal show_speech(text)
signal hide_speech()

signal show_narration(text)
signal hide_narration()
signal hide_narration_simple()

signal show_player_info(text)
signal hide_player_info()

signal change_dad_max_dist(dist)

signal dad_repeat_log(state)
signal log_emit()

signal clearing_trigger_orb()

signal father_gone()

signal orb_next_position()

signal orb_sense_player(state)

signal night_path_set_up()
signal orb_to_night_path()

signal cave_path_set_up()

signal fork_set_up()

signal dad_call()

signal follow_voice()

signal stick_drop_forest()

signal animal_trigger()

signal hiding()

signal stick_create()

signal stick_reset()

signal cave_path_trigger()

signal voice_path_reset()

signal cave_path_reset()

signal lake_path_reset()

signal item_collected(item)

signal light_state(state)

signal buzz_home_set(home)

signal buzz_active(state)

signal buzz_attack(state)

signal dark_place(state)

signal caught_by_buzz()

signal path_chosen(path)

signal buzz_off()


signal orb_to_clearing_two(start_state)
signal voice_to_clearing_two(start_state)

signal pull_into_lake()

signal start_lake()

signal dad_lake_reset()
signal orb_lake_reset()

signal animal_to_lake()

signal read(text)

signal mouse_capture(state)

signal mouse_capture_read(state)

signal orb_lake_player_caught()
signal voice_lake_player_caught()

signal stop_dad_calling()

signal dad_use_log()

signal drop_dome(state)

signal dad_audio(audio)

signal dad_end_call()

signal picked_up_timed_throw_item()

signal time_transition_0
signal time_transition_1
signal time_transition_2
signal time_transition_3
signal time_transition_4
signal time_transition_5
signal time_transition_6
signal time_transition_7
signal time_transition_8
signal time_transition_9
